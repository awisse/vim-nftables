...
syn match nftSetProperty "\<timeout\s\+\d\+\%(\.\d*\)[dhms]\>" contains=nftSetTimeout

syn match nftElementProperty "\<\%(expires\|timeout\)\s\+\%(\d\{1,2}d\)\?\%(\d\{1,2}h\)\?\%(\d\{1,2}m\)\?\%(\d\{1,2}s\?\)\?\>" contains=nftElementTimeout
...
syn match   nftSetTimeout   "\<\d\+\%(\.\d*\)\?[dhms]\>" contained
syn match   nftElementTimeout "\<\%(\d\{1,2}d\)\?\%(\d\{1,2}h\)\?\%(\d\{1,2}m\)\?\%(\d\{1,2}s\?\)\?\>" contained
...
hi link nftElementTimeout nftValue
